...
export const worklog: AiWorklog = {
  ...
  stages: [{
    ...
    aiWork: ['Добавил типизированный запрос заявок, состояния загрузки, ошибки и пустого списка, а также адаптивные карточки.'],
    keyPrompts: ['Реализовать список заявок с именем, тревожностью, температурой, дедлайном и особыми условиями.'],
    outcome: 'Список получает данные из mock API; метрики времени и токенов намеренно не выдуманы.',
  }, {
    name: 'Роутинг',
    durationMinutes: null,
    developerWork: ['Время этапа отдельно не фиксировалось.'],
    aiWork: ['Добавил конфигурацию React Router, layout с навигацией и fallback-страницу.'],
    keyPrompts: ['Реализовать роутинг React Router, вдохновляясь router.tsx из booking-rooms.'],
    outcome: 'Маршруты /, /places, /report, /ai-worklog и fallback определены в app/router.tsx.',
  }, {
    name: 'Визуальный стиль',
    durationMinutes: null,
    developerWork: ['Время этапа отдельно не фиксировалось.'],
    aiWork: ['Перевёл интерфейс на Tailwind и локальные shadcn-компоненты, добавив states для списка.'],
    keyPrompts: ['Возобновить дизайн с установленными Tailwind и shadcn.'],
    outcome: 'Ручные стили заменены Tailwind-классами; компоненты добавлены через shadcn CLI.',
  }, {
    name: 'Рефакторинг состояний списка',
    durationMinutes: null,
    developerWork: ['Запросил заменить цепочку if для состояний списка на диспетчеризацию по состоянию.'],
    aiWork: ['Разделил представления pending, error, empty и ready; выбор представления выполняется через switch.'],
    keyPrompts: ['Применить паттерн RoomsScreen для состояний RelocationBoard.'],
    outcome: 'Поведение списка сохранено; для неожиданного состояния есть защитная заглушка. yarn lint и yarn build проходят.',
  }, {
    name: 'Качество кода: порядок импортов',
    durationMinutes: null,
    developerWork: ['Запросил единый порядок импортов.'],
    aiWork: ['Подключил ESLint-правило с автоматическим исправлением порядка импортов.'],
    keyPrompts: ['Добавить ESLint-правило по сортировке импортов.'],
    outcome: 'eslint-plugin-simple-import-sort добавлен в devDependencies; проверка выполняется через yarn lint.',
  }, {
    name: 'Рефакторинг карточки заявки',
    durationMinutes: null,
    developerWork: ['Запросил вынести вычисления карточки заявки в model.'],
    aiWork: ['Перенёс формирование условий и форматирование дедлайна в чистые функции сущности.'],
    keyPrompts: ['Вынести вычисления из GhostApplicationCard в model.'],
    outcome: 'UI-компонент отвечает только за отображение; поведение сохранено.',
  }],
