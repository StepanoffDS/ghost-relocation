...
const defaultState: MockState = {
  ...
  relocations: emptyRelocations(defaultGhosts),
};
...
const fullOwner: Ghost = {
  ...defaultGhosts[0],
  id: ids.agatha,
  name: 'Занявшая место Агата',
};
const fullGuest: Ghost = {
  ...defaultGhosts[2],
  id: ids.boris,
  name: 'Ждёт освобождения Борис',
};
const fullState: MockState = {
  ...
  places: [
    {
      id: ids.basement,
      name: 'Единственный сухой подвал',
      type: 'basement',
      capacity: 1,
      temperature: 10,
      lighting: 'low',
  ...
  relocations: [
    {
      ghostId: fullOwner.id,
      placeId: ids.basement,
      mode: 'manual',
      score: 100,
      issues: [],
    },
    {
      ghostId: fullGuest.id,
      placeId: null,
      mode: 'unassigned',
      score: null,
      issues: [],
...
export const createFixture = (fixture: DemoFixture): MockState =>
  structuredClone(fixtures[fixture]);
